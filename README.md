...
| Surface | Value |
| --- | --- |
| Adapter version | `1.0.13` |
| Runtime baseline | MiMoCode `0.1.0` |
| GitHub release tag | `1.0.13` |
| Pinned commit | `b05e5d79826abc96c9323339420d2dd7e8064918` |
...
```bash
curl -fsSL https://mimo.xiaomi.com/install | bash -s -- --version 0.1.0 --no-modify-path
```
...
```bash
npm install -g @mimo-ai/cli@0.1.0
```
...
| chrome-devtools | local | `chrome-devtools-mcp@1.2.0` via `bunx` |
...
| context7 | local | `@upstash/context7-mcp@3.2.1` via `bunx` |
...
- **Playwright CLI** - deterministic screenshots, snapshots, traces, and visual
  evidence.
- **Chrome DevTools MCP** (`chrome-devtools-mcp@1.2.0`) - console, network,
